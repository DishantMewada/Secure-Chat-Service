...
//	}
	...
	public DH() throws Exception{
		...
		// on machine 1
             str=new String("994642092651365954479983077845914076425057093616322438024678641058248197737719377778071711754640873228204522093021225552584637320850272672695704121263066");
	secretA = new BigInteger(str.getBytes());
	    //secretA = new BigInteger(bitLength-2,randomGenerator);
            //str = secretA.toString(153);
            //str = new String(secretA.toByteArray());
            //secretA = new BigInteger(str.getBytes()); 
...
BigInteger findPrime(){
	 Random rnd=new Random();
        //rnd = (Random) rnd.ints(certainty, certainty+1);
	BigInteger p;
	// while(!isPrime(p))
